namespace EnergyTrading.MDM.Extensions
{
    ...
    {
        public static EnergyTrading.Contracts.Search.Search ToSearch<TContract>(this string key)
        {
            var searchString = FromBase64(key.Replace("-", "/").Replace(")", "+").Replace("(", "=")).Substring(typeof(TContract).Name.Length);
            return searchString.DeserializeDataContractXmlString<EnergyTrading.Contracts.Search.Search>();
        }
        ...
        public static string ToKey<TContract>(this EnergyTrading.Contracts.Search.Search search)
        {
