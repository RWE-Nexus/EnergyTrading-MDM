namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductTypeInstanceUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<ProductTypeInstance>(1)).Returns(new ProductTypeInstance());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.ProductTypeInstanceDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.ProductTypeInstance { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<ProductTypeInstance>();
            details.Id = 1;
            var entity = ObjectMother.Create<ProductTypeInstance>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<ProductTypeInstance>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new ProductTypeInstanceMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<ProductTypeInstance>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<ProductTypeInstance>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.ProductTypeInstanceDetails, ProductTypeInstance>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, ProductTypeInstanceMapping>(identifier)).Returns(mapping);
