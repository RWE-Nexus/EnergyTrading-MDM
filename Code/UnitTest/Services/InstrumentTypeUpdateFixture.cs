namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class InstrumentTypeUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<InstrumentType>(1)).Returns(new InstrumentType());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.InstrumentTypeDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.InstrumentType { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<InstrumentType>();
            details.Id = 1;
            var entity = ObjectMother.Create<InstrumentType>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<InstrumentType>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new InstrumentTypeMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<InstrumentType>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<InstrumentType>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.InstrumentTypeDetails, InstrumentType>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, InstrumentTypeMapping>(identifier)).Returns(mapping);
