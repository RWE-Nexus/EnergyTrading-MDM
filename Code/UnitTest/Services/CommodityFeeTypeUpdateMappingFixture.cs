namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class CommodityFeeTypeUpdateMappingFixture
    {
        [TestMethod]
        ...
        public void VersionConflict()
        {
            ...
            var mapping = new CommodityFeeTypeMapping { CommodityFeeType = new MDM.CommodityFeeType() { Timestamp = BitConverter.GetBytes(25L) } };
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<AmendMappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            repository.Setup(x => x.FindOne<CommodityFeeTypeMapping>(12)).Returns(mapping);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var m1 = new CommodityFeeTypeMapping { Id = mappingId, System = s1, MappingValue = "1", Version = BitConverter.GetBytes(34L), Validity = new DateRange(start, DateUtility.MaxDate) };			
            var m2 = new CommodityFeeTypeMapping { Id = mappingId, System = s1, MappingValue = "1", Validity = new DateRange(start, finish) };

            // NB We deliberately bypasses the business logic
            var entity = new MDM.CommodityFeeType();
            m1.CommodityFeeType = entity;
            entity.Mappings.Add(m1);

            validatorFactory.Setup(x => x.IsValid(It.IsAny<AmendMappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            repository.Setup(x => x.FindOne<CommodityFeeTypeMapping>(mappingId)).Returns(m1);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, CommodityFeeTypeMapping>(identifier)).Returns(m2);
            ...
            // NB Don't verify result of Update - already covered by CommodityFeeTypeMappingFixture
