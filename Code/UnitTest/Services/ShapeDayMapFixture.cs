namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ShapeDayMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<ShapeDayMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<ShapeDayMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new ShapeDayMapping
            {
                System = system,
                MappingValue = "A"
            };
            ...
            //ShapeDay.AddDetails(details);
            shapeday.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.ShapeDayDetails();

            mappingEngine.Setup(x => x.Map<ShapeDayMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<ShapeDay, RWEST.Nexus.MDM.Contracts.ShapeDayDetails>(shapeday)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<ShapeDayMapping> { mapping };
            repository.Setup(x => x.Queryable<ShapeDayMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<ShapeDayMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<ShapeDay, RWEST.Nexus.MDM.Contracts.ShapeDayDetails>(shapeday));
            repository.Verify(x => x.Queryable<ShapeDayMapping>());
