namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ShipperCodeMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            // Assert
            repository.Verify(x => x.Queryable<ShipperCodeMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var shippercode = new ShipperCode
            {
                Id = 1,
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.ShipperCodeDetails();

            mappingEngine.Setup(x => x.Map<ShipperCodeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<ShipperCode, RWEST.Nexus.MDM.Contracts.ShipperCodeDetails>(shippercode)).Returns(cDetails);
            validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<ShipperCodeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<ShipperCode, RWEST.Nexus.MDM.Contracts.ShipperCodeDetails>(shippercode));
            repository.Verify(x => x.Queryable<ShipperCodeMapping>());
