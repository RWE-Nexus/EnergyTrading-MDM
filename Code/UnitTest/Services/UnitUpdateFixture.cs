namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class UnitUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<Unit>(1)).Returns(new Unit());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.UnitDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.Unit { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<Unit>();
            details.Id = 1;
            var entity = ObjectMother.Create<Unit>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<Unit>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new UnitMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<Unit>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<Unit>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.UnitDetails, Unit>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, UnitMapping>(identifier)).Returns(mapping);
