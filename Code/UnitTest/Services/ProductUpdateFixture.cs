namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<Product>(1)).Returns(new Product());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.ProductDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.Product { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<Product>();
            details.Id = 1;
            var entity = ObjectMother.Create<Product>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<Product>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new ProductMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<Product>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<Product>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.ProductDetails, Product>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, ProductMapping>(identifier)).Returns(mapping);
