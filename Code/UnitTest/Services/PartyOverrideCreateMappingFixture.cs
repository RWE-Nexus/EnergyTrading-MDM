namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class PartyOverrideCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.PartyOverride>(12)).Returns(partyoverride);

            // Act
            var candidate = (PartyOverrideMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(partyoverride));
