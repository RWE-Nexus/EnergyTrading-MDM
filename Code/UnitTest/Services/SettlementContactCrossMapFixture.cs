namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class SettlementContactCrossMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<PartyAccountabilityMapping>();
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new PartyAccountabilityMapping
                {
                    System = system,
                    MappingValue = "A"
                };
            var targetSystem = new MDM.SourceSystem { Name = "Trayport" };
            var targetMapping = new PartyAccountabilityMapping
                {
                    System = targetSystem,
                    MappingValue = "B",
                    IsDefault = true
                };
            var details = new MDM.SettlementContact
                {
                    Name = "Test",
                };
            var settlementcontact = new MDM.SettlementContact
                {
                    Id = 1
                };
            settlementcontact.AddDetails(details);
            settlementcontact.ProcessMapping(mapping);
            settlementcontact.ProcessMapping(targetMapping);
            ...
            mappingEngine.Setup(x => x.Map<PartyAccountabilityMapping, NexusId>(targetMapping)).Returns(targetIdentifier);

            var list = new List<PartyAccountabilityMapping> { mapping };
            repository.Setup(x => x.Queryable<PartyAccountabilityMapping>()).Returns(list.AsQueryable());
