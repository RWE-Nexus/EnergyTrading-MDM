namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class BrokerRateCreateFixture
    {
        [TestMethod]
        ...
        public void ValidContractIsSaved()
        {
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.BrokerRate, BrokerRate>(contract)).Returns(brokerrate);

            // Act
            var expected = service.Create(contract);

            // Assert
            Assert.AreSame(expected, brokerrate, "BrokerRate differs");
            repository.Verify(x => x.Add(brokerrate));
