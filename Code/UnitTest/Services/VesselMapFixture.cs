namespace EnergyTrading.MDM.Test.Services
{
    ...
    public class VesselMapFixture
    {
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<VesselMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<VesselMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            //Vessel.AddDetails(details);
            vessel.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.VesselDetails();

            mappingEngine.Setup(x => x.Map<VesselMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<Vessel, RWEST.Nexus.MDM.Contracts.VesselDetails>(vessel)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<VesselMapping> { mapping };
            repository.Setup(x => x.Queryable<VesselMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<VesselMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<Vessel, RWEST.Nexus.MDM.Contracts.VesselDetails>(vessel));
            repository.Verify(x => x.Queryable<VesselMapping>());
