namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class BusinessUnitUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<RWEST.Nexus.MDM.Contracts.BusinessUnit>(), It.IsAny<IList<IRule>>())).Returns(true);

            var service = new BusinessUnitService(validatorFactory.Object, mappingEngine.Object, repository.Object, searchCache.Object);

            var cd = new RWEST.Nexus.MDM.Contracts.BusinessUnitDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.BusinessUnit { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<BusinessUnitDetails>();
            details.Id = 1;
            var entity = ObjectMother.Create<BusinessUnit>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<BusinessUnit>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var d2 = new BusinessUnitDetails {Name = "BU 1" };
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.BusinessUnitDetails, BusinessUnitDetails>(cd)).Returns(d2);
