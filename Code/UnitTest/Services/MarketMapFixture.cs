namespace EnergyTrading.MDM.Test.Services
{
    ...
    public class MarketMapFixture
    {
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<MarketMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<MarketMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            //Market.AddDetails(details);
            market.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.MarketDetails();

            mappingEngine.Setup(x => x.Map<MarketMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<Market, RWEST.Nexus.MDM.Contracts.MarketDetails>(market)).Returns(cDetails);
            validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);

            var list = new List<MarketMapping> { mapping };
            repository.Setup(x => x.Queryable<MarketMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<MarketMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<Market, RWEST.Nexus.MDM.Contracts.MarketDetails>(market));
            repository.Verify(x => x.Queryable<MarketMapping>());
