namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ShapeDayCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.ShapeDay>(12)).Returns(shapeday);

            // Act
            var candidate = (ShapeDayMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(shapeday));
