namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class PortfolioHierarchyMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<PortfolioHierarchyMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<PortfolioHierarchyMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new PortfolioHierarchyMapping
            {
                System = system,
                MappingValue = "A"
            };
            ...
            //PortfolioHierarchy.AddDetails(details);
            portfoliohierarchy.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.PortfolioHierarchyDetails();

            mappingEngine.Setup(x => x.Map<PortfolioHierarchyMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<PortfolioHierarchy, RWEST.Nexus.MDM.Contracts.PortfolioHierarchyDetails>(portfoliohierarchy)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<PortfolioHierarchyMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<PortfolioHierarchy, RWEST.Nexus.MDM.Contracts.PortfolioHierarchyDetails>(portfoliohierarchy));
            repository.Verify(x => x.Queryable<PortfolioHierarchyMapping>());
