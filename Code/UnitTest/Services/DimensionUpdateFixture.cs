namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class DimensionUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<Dimension>(1)).Returns(new Dimension());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.DimensionDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.Dimension { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<Dimension>();
            details.Id = 1;
            var entity = ObjectMother.Create<Dimension>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<Dimension>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new DimensionMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<Dimension>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<Dimension>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.DimensionDetails, Dimension>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, DimensionMapping>(identifier)).Returns(mapping);
