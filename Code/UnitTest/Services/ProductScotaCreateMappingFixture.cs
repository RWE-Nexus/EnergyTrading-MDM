namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductScotaCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            var productscota = new MDM.ProductScota();
            repository.Setup(x => x.FindOne<MDM.ProductScota>(12)).Returns(productscota);

            // Act
            var candidate = (ProductScotaMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(productscota));
