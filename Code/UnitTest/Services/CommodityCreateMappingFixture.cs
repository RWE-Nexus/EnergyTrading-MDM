namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class CommodityCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            var commodity = new MDM.Commodity();
            repository.Setup(x => x.FindOne<MDM.Commodity>(12)).Returns(commodity);

            // Act
            var candidate = (CommodityMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(commodity));
