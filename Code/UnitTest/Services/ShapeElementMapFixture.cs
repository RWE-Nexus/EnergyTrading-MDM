namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ShapeElementMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            // Assert
            repository.Verify(x => x.Queryable<ShapeElementMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.ShapeElementDetails();

            mappingEngine.Setup(x => x.Map<ShapeElementMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<ShapeElement, RWEST.Nexus.MDM.Contracts.ShapeElementDetails>(shapeelement)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<ShapeElementMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<ShapeElement, RWEST.Nexus.MDM.Contracts.ShapeElementDetails>(shapeelement));
            repository.Verify(x => x.Queryable<ShapeElementMapping>());
