namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductCurveUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<ProductCurve>(1)).Returns(new ProductCurve());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<RWEST.Nexus.MDM.Contracts.ProductCurve>(), It.IsAny<IList<IRule>>())).Returns(true);

            var service = new ProductCurveService(validatorFactory.Object, mappingEngine.Object, repository.Object, searchCache.Object);

            var cd = new RWEST.Nexus.MDM.Contracts.ProductCurveDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.ProductCurve { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<ProductCurve>();
            details.Id = 1;
            var entity = ObjectMother.Create<ProductCurve>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<ProductCurve>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var d1 = ObjectMother.Create<ProductCurve>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<ProductCurve>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.ProductCurveDetails, ProductCurve>(cd)).Returns(d2);
