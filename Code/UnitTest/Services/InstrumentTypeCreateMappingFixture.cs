namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class InstrumentTypeCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.InstrumentType>(12)).Returns(instrumenttype);

            // Act
            var candidate = (InstrumentTypeMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(instrumenttype));
