namespace EnergyTrading.MDM.Test.Services
{
    ...
    public class LocationMapFixture
    {
        ...
        public void SuccessMatch()
        {
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.LocationDetails();

            mappingEngine.Setup(x => x.Map<LocationMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<Location, RWEST.Nexus.MDM.Contracts.LocationDetails>(location)).Returns(cDetails);
            validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<LocationMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<Location, RWEST.Nexus.MDM.Contracts.LocationDetails>(location));
            repository.Verify(x => x.Queryable<LocationMapping>());
