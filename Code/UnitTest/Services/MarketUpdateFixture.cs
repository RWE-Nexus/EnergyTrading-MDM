namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class MarketUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<Market>(1)).Returns(new Market());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.MarketDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.Market { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<Market>();
            details.Id = 1;
            var entity = ObjectMother.Create<Market>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<Market>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new MarketMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<Market>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<Market>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.MarketDetails, Market>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, MarketMapping>(identifier)).Returns(mapping);
