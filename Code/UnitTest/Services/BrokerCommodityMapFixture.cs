namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class BrokerCommodityMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<BrokerCommodityMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<BrokerCommodityMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            //BrokerCommodity.AddDetails(details);
            brokercommodity.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.BrokerCommodityDetails();

            mappingEngine.Setup(x => x.Map<BrokerCommodityMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<BrokerCommodity, RWEST.Nexus.MDM.Contracts.BrokerCommodityDetails>(brokercommodity)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<BrokerCommodityMapping> { mapping };
            repository.Setup(x => x.Queryable<BrokerCommodityMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<BrokerCommodityMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<BrokerCommodity, RWEST.Nexus.MDM.Contracts.BrokerCommodityDetails>(brokercommodity));
            repository.Verify(x => x.Queryable<BrokerCommodityMapping>());
