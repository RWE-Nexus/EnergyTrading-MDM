namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class PortfolioMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<PortfolioMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<PortfolioMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            //Portfolio.AddDetails(details);
            portfolio.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.PortfolioDetails();

            mappingEngine.Setup(x => x.Map<PortfolioMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<Portfolio, RWEST.Nexus.MDM.Contracts.PortfolioDetails>(portfolio)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<PortfolioMapping> { mapping };
            repository.Setup(x => x.Queryable<PortfolioMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<PortfolioMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<Portfolio, RWEST.Nexus.MDM.Contracts.PortfolioDetails>(portfolio));
            repository.Verify(x => x.Queryable<PortfolioMapping>());
