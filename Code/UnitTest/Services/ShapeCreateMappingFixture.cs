namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ShapeCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.Shape>(12)).Returns(shape);

            // Act
            var candidate = (ShapeMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(shape));
