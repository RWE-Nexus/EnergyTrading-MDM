namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class PartyOverrideUpdateMappingFixture
    {
        [TestMethod]
        ...
        public void VersionConflict()
        {
            ...
            var mapping = new PartyOverrideMapping { PartyOverride = new MDM.PartyOverride() { Timestamp = BitConverter.GetBytes(25L) } };
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<AmendMappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            repository.Setup(x => x.FindOne<PartyOverrideMapping>(12)).Returns(mapping);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var m1 = new PartyOverrideMapping { Id = mappingId, System = s1, MappingValue = "1", Version = BitConverter.GetBytes(34L), Validity = new DateRange(start, DateUtility.MaxDate) };			
            var m2 = new PartyOverrideMapping { Id = mappingId, System = s1, MappingValue = "1", Validity = new DateRange(start, finish) };

            // NB We deliberately bypasses the business logic
            var entity = new MDM.PartyOverride();
            m1.PartyOverride = entity;
            entity.Mappings.Add(m1);

            validatorFactory.Setup(x => x.IsValid(It.IsAny<AmendMappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            repository.Setup(x => x.FindOne<PartyOverrideMapping>(mappingId)).Returns(m1);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, PartyOverrideMapping>(identifier)).Returns(m2);
            ...
            // NB Don't verify result of Update - already covered by PartyOverrideMappingFixture
