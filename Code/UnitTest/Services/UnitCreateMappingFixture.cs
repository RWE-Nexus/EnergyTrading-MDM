namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class UnitCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            var unit = new MDM.Unit();
            repository.Setup(x => x.FindOne<MDM.Unit>(12)).Returns(unit);

            // Act
            var candidate = (UnitMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(unit));
