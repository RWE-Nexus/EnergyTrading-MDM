namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductTypeUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<ProductType>(1)).Returns(new ProductType());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.ProductTypeDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.ProductType { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<ProductType>();
            details.Id = 1;
            var entity = ObjectMother.Create<ProductType>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<ProductType>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new ProductTypeMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<ProductType>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<ProductType>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.ProductTypeDetails, ProductType>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, ProductTypeMapping>(identifier)).Returns(mapping);
