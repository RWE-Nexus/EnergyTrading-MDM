namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class VesselUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<Vessel>(1)).Returns(new Vessel());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.VesselDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.Vessel { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<Vessel>();
            details.Id = 1;
            var entity = ObjectMother.Create<Vessel>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<Vessel>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new VesselMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<Vessel>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<Vessel>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.VesselDetails, Vessel>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, VesselMapping>(identifier)).Returns(mapping);
