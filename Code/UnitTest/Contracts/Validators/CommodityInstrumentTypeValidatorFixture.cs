namespace EnergyTrading.MDM.Test.Contracts.Validators
{
    ...
    using CommodityInstrumentType = RWEST.Nexus.MDM.Contracts.CommodityInstrumentType;

    [TestClass]
    public partial class CommodityInstrumentTypeValidatorFixture : Fixture
    {
        [TestMethod]
        public void ValidatorResolution()
        {
            ...
            var config = new CommodityInstrumentTypeConfiguration(container);
            config.Configure();

            var validator = container.Resolve<IValidator<CommodityInstrumentType>>("commodityinstrumenttype");
        ...
        [TestMethod]
        public void ValidCommodityInstrumentTypePasses()
        {
            // Assert
            ...
            var validator = new CommodityInstrumentTypeValidator(validatorEngine.Object, repository);

            var commodityinstrumenttype = new CommodityInstrumentType { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { identifier } };
            this.AddRelatedEntities(commodityinstrumenttype);

            // Act
            var violations = new List<IRule>();
            var result = validator.IsValid(commodityinstrumenttype, violations);
        ...
        public void OverlapsRangeFails()
        {
            ...
            var system = new MDM.SourceSystem { Name = "Test" };
            var commodityinstrumenttypeMapping = new CommodityInstrumentTypeMapping { System = system, MappingValue = "1", Validity = validity };

            var list = new List<CommodityInstrumentTypeMapping> { commodityinstrumenttypeMapping };
            var repository = new Mock<IRepository>();
            repository.Setup(x => x.Queryable<CommodityInstrumentTypeMapping>()).Returns(list.AsQueryable());
            ...
            var identifierValidator = new NexusIdValidator<CommodityInstrumentTypeMapping>(repository.Object);
            var validatorEngine = new Mock<IValidatorEngine>();
            ...
            var validator = new CommodityInstrumentTypeValidator(validatorEngine.Object, repository.Object);

            var commodityinstrumenttype = new CommodityInstrumentType { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { overlapsRangeIdentifier } };
        ...
        public void BadSystemFails()
        {
            ...
            var commodityinstrumenttype = new CommodityInstrumentType { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { badSystemIdentifier } };
