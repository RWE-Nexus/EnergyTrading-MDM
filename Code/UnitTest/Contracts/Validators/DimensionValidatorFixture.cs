namespace EnergyTrading.MDM.Test.Contracts.Validators
{
    ...
    using Dimension = RWEST.Nexus.MDM.Contracts.Dimension;

    [TestClass]
    public partial class DimensionValidatorFixture : Fixture
    {
        [TestMethod]
        public void ValidatorResolution()
        {
            ...
            var config = new DimensionConfiguration(container);
            config.Configure();

            var validator = container.Resolve<IValidator<Dimension>>("dimension");
        ...
        [TestMethod]
        public void ValidDimensionPasses()
        {
            // Assert
            ...
            var validator = new DimensionValidator(validatorEngine.Object, repository);

            var dimension = new Dimension { Details = new RWEST.Nexus.MDM.Contracts.DimensionDetails{Name = "Test"}, Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { identifier } };
            this.AddRelatedEntities(dimension);

            // Act
            var violations = new List<IRule>();
            var result = validator.IsValid(dimension, violations);
        ...
        public void OverlapsRangeFails()
        {
            ...
            var system = new MDM.SourceSystem { Name = "Test" };
            var dimensionMapping = new DimensionMapping { System = system, MappingValue = "1", Validity = validity };

            var list = new List<DimensionMapping> { dimensionMapping };
            var repository = new Mock<IRepository>();
            repository.Setup(x => x.Queryable<DimensionMapping>()).Returns(list.AsQueryable());
            ...
            var identifierValidator = new NexusIdValidator<DimensionMapping>(repository.Object);
            var validatorEngine = new Mock<IValidatorEngine>();
            ...
            var validator = new DimensionValidator(validatorEngine.Object, repository.Object);

            var dimension = new Dimension { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { overlapsRangeIdentifier } };
        ...
        public void BadSystemFails()
        {
            ...
            var dimension = new Dimension { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { badSystemIdentifier } };
		...
		partial void AddRelatedEntities(RWEST.Nexus.MDM.Contracts.Dimension contract);
