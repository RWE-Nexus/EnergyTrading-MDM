namespace EnergyTrading.MDM.Test.Contracts.Validators
{
    ...
    using Agreement = RWEST.Nexus.MDM.Contracts.Agreement;

    [TestClass]
    public partial class AgreementValidatorFixture : Fixture
    {
        [TestMethod]
        public void ValidatorResolution()
        {
            ...
            var config = new AgreementConfiguration(container);
            config.Configure();

            var validator = container.Resolve<IValidator<Agreement>>("agreement");
        ...
        [TestMethod]
        public void ValidAgreementPasses()
        {
            // Assert
            ...
            var validator = new AgreementValidator(validatorEngine.Object, repository);

            var agreement = new Agreement { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { identifier } };
            this.AddRelatedEntities(agreement);

            // Act
            var violations = new List<IRule>();
            var result = validator.IsValid(agreement, violations);
        ...
        public void OverlapsRangeFails()
        {
            ...
            var system = new MDM.SourceSystem { Name = "Test" };
            var agreementMapping = new AgreementMapping { System = system, MappingValue = "1", Validity = validity };

            var list = new List<AgreementMapping> { agreementMapping };
            var repository = new Mock<IRepository>();
            repository.Setup(x => x.Queryable<AgreementMapping>()).Returns(list.AsQueryable());
            ...
            var identifierValidator = new NexusIdValidator<AgreementMapping>(repository.Object);
            var validatorEngine = new Mock<IValidatorEngine>();
            ...
            var validator = new AgreementValidator(validatorEngine.Object, repository.Object);

            var agreement = new Agreement { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { overlapsRangeIdentifier } };
        ...
        public void BadSystemFails()
        {
            ...
            var agreement = new Agreement { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { badSystemIdentifier } };
        ...
        partial void AddRelatedEntities(RWEST.Nexus.MDM.Contracts.Agreement contract);
