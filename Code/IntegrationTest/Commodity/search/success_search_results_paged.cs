	namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_search_for_a_commodity_with_two_results_is_paged_at_one_per_page : IntegrationTestBase
    {
        private static HttpClient client;

        private static HttpContent content;

        private static MDM.Commodity entity1;
        private static MDM.Commodity entity2;

        private static IList<Commodity> pageOne;
        private static Uri pageOneNextPage;
        private static IList<Commodity> pageTwo;
        ...
        {
            pageOneResponse = client.Post(ServiceUrl["Commodity"] + "search", content);
        ...
        {
            entity1 = Script.CommodityData.CreateBasicEntity();
            entity2 = Script.CommodityData.CreateBasicEntity();

            client = new HttpClient();

			var search = new Search().IsMutliPage().MaxPageSize(1);
			Script.CommodityData.CreateSearch(search, entity1, entity2);
        ...
        private static IList<Commodity> GetPeopleFromFeed(SyndicationFeed feed)
        {
            List<RWEST.Nexus.MDM.Contracts.Commodity> result =
                feed.Items.Select(syndicationItem => (XmlSyndicationContent)syndicationItem.Content).Select(
                    syndic => syndic.ReadContent<RWEST.Nexus.MDM.Contracts.Commodity>()).ToList();
