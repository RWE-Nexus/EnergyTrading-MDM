namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_search_for_a_productscota_is_made_with_a_mapping_value_and_results_are_found : IntegrationTestBase
    {
        ...
        public void should_return_the_relevant_search_results()
        {
            ...
            List<RWEST.Nexus.MDM.Contracts.ProductScota> result =
                feed.Items.Select(syndicationItem => (XmlSyndicationContent)syndicationItem.Content).Select(
                    syndic => syndic.ReadContent<RWEST.Nexus.MDM.Contracts.ProductScota>()).ToList();
        ...
        {
            response = client.Post(ServiceUrl["ProductScota"] + "search", content);
        }

        protected static void Establish_context()
        {
            entity1 = ProductScotaData.CreateBasicEntityWithOneMapping();
            entity2 = ProductScotaData.CreateBasicEntityWithOneMapping();
    ...
    [TestClass]
    public class when_a_search_for_a_productscota_is_made_with_a_mapping_value_and_a_system_name_and_results_are_found : IntegrationTestBase
    {
        private static HttpClient client;
