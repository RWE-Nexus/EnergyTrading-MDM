	namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_search_for_a_vessel_with_two_results_is_paged_at_one_per_page : IntegrationTestBase
    {
        ...
        private static IList<Vessel> pageOne;
        private static Uri pageOneNextPage;
        private static IList<Vessel> pageTwo;
        ...
        {
            pageOneResponse = client.Post(ServiceUrl["Vessel"] + "search", content);
        ...
        protected static void Establish_context()
        {
			...
			VesselData.CreateSearch(search, entity1, entity2);
        ...
        private static IList<Vessel> GetPeopleFromFeed(SyndicationFeed feed)
        {
            List<RWEST.Nexus.MDM.Contracts.Vessel> result =
                feed.Items.Select(syndicationItem => (XmlSyndicationContent)syndicationItem.Content).Select(
                    syndic => syndic.ReadContent<RWEST.Nexus.MDM.Contracts.Vessel>()).ToList();
