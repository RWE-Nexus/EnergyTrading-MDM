namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_request_is_made_to_create_a_producttenortype_mapping : IntegrationTestBase
    {
        ...
        {
            response = client.Post(ServiceUrl["ProductTenorType"] + string.Format("{0}/Mapping", entity.Id), content);
        }

        [TestMethod]
        public void should_create_a_mapping_on_the_producttenortype_entity()
        {
            var savedMapping =
                new DbSetRepository<MDM.ProductTenorTypeMapping>(new NexusMappingContext()).FindOne(int.Parse(GetLocationHeader()[3]));
        ...
        public void should_return_the_location_of_the_entity()
        {
            ...
            Assert.IsTrue(parsedEntityId, "The ProductTenorType id returned was not an integer");
