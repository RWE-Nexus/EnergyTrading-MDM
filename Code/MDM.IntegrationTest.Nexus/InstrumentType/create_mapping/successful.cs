namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_request_is_made_to_create_a_instrumenttype_mapping : IntegrationTestBase
    {
        ...
        {
            response = client.Post(ServiceUrl["InstrumentType"] + string.Format("{0}/Mapping", entity.Id), content);
        }

        [TestMethod]
        public void should_create_a_mapping_on_the_instrumenttype_entity()
        {
            var savedMapping =
                new DbSetRepository<MDM.InstrumentTypeMapping>(new NexusMappingContext()).FindOne(int.Parse(GetLocationHeader()[3]));
        ...
        public void should_return_the_location_of_the_entity()
        {
            ...
            Assert.IsTrue(parsedEntityId, "The InstrumentType id returned was not an integer");
