	namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_search_for_a_partyaccountability_with_two_results_is_paged_at_one_per_page : IntegrationTestBase
    {
        private static HttpClient client;

        private static HttpContent content;

        private static MDM.PartyAccountability entity1;
        private static MDM.PartyAccountability entity2;

        private static IList<PartyAccountability> pageOne;
        private static Uri pageOneNextPage;
        private static IList<PartyAccountability> pageTwo;
        ...
        {
            pageOneResponse = client.Post(ServiceUrl["PartyAccountability"] + "search", content);
        ...
        {
            entity1 = PartyAccountabilityData.CreateBasicEntity();
            entity2 = PartyAccountabilityData.CreateBasicEntity();

            client = new HttpClient();

			var search = new Search().IsMutliPage().MaxPageSize(1);
			PartyAccountabilityData.CreateSearch(search, entity1, entity2);
        ...
        private static IList<PartyAccountability> GetPeopleFromFeed(SyndicationFeed feed)
        {
            List<RWEST.Nexus.MDM.Contracts.PartyAccountability> result =
                feed.Items.Select(syndicationItem => (XmlSyndicationContent)syndicationItem.Content).Select(
                    syndic => syndic.ReadContent<RWEST.Nexus.MDM.Contracts.PartyAccountability>()).ToList();
