namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_request_is_made_to_create_a_producttype_mapping : IntegrationTestBase
    {
        ...
        {
            response = client.Post(ServiceUrl["ProductType"] + string.Format("{0}/Mapping", entity.Id), content);
        }

        [TestMethod]
        public void should_create_a_mapping_on_the_producttype_entity()
        {
            var savedMapping =
                new DbSetRepository<MDM.ProductTypeMapping>(new NexusMappingContext()).FindOne(int.Parse(GetLocationHeader()[3]));
        ...
        {
            //Assert.AreEqual("ProductType", GetLocationHeader()[0], true);
            int id;
            bool parsedEntityId = int.TryParse(GetLocationHeader()[1], out id);
            bool parsedMappingId = int.TryParse(GetLocationHeader()[3], out id);
            Assert.IsTrue(parsedEntityId, "The ProductType id returned was not an integer");
