namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_request_is_made_to_create_a_calendar_mapping : IntegrationTestBase
    {
        ...
        {
            response = client.Post(ServiceUrl["Calendar"] + string.Format("{0}/Mapping", entity.Id), content);
        }

        [TestMethod]
        public void should_create_a_mapping_on_the_calendar_entity()
        {
            var savedMapping =
                new DbSetRepository<MDM.CalendarMapping>(new NexusMappingContext()).FindOne(int.Parse(GetLocationHeader()[3]));
        ...
        {
            // Assert.AreEqual("Calendar", GetLocationHeader()[0], true);
            int id;
            bool parsedEntityId = int.TryParse(GetLocationHeader()[1], out id);
            bool parsedMappingId = int.TryParse(GetLocationHeader()[3], out id);
            Assert.IsTrue(parsedEntityId, "The Calendar id returned was not an integer");
