namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_request_is_made_to_create_a_brokerrate_mapping : IntegrationTestBase
    {
        ...
        {
            response = client.Post(ServiceUrl["BrokerRate"] + string.Format("{0}/Mapping", entity.Id), content);
        }

        [TestMethod]
        public void should_create_a_mapping_on_the_brokerrate_entity()
        {
            var savedMapping =
                new DbSetRepository<MDM.BrokerRateMapping>(new NexusMappingContext()).FindOne(int.Parse(GetLocationHeader()[3]));
        ...
        public void should_return_the_location_of_the_entity()
        {
            ...
            Assert.IsTrue(parsedEntityId, "The BrokerRate id returned was not an integer");
