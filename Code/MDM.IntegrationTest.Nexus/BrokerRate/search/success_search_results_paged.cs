    namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    [Ignore] // Ignored becuase broker rate search doen't support paging, it's a direct search
    public class when_a_search_for_a_brokerrate_with_two_results_is_paged_at_one_per_page : IntegrationTestBase
    {
        ...
        private static IList<BrokerRate> pageOne;
        private static Uri pageOneNextPage;
        private static IList<BrokerRate> pageTwo;
        ...
        {
            pageOneResponse = client.Post(ServiceUrl["BrokerRate"] + "search", content);
        ...
        protected static void Establish_context()
        {
            ...
            BrokerRateData.CreateSearch(search, entity1, entity2);
        ...
        private static IList<BrokerRate> GetPeopleFromFeed(SyndicationFeed feed)
        {
            List<RWEST.Nexus.MDM.Contracts.BrokerRate> result =
                feed.Items.Select(syndicationItem => (XmlSyndicationContent)syndicationItem.Content).Select(
                    syndic => syndic.ReadContent<RWEST.Nexus.MDM.Contracts.BrokerRate>()).ToList();
