namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_request_is_made_to_create_a_commodity_mapping : IntegrationTestBase
    {
        ...
        {
            response = client.Post(ServiceUrl["Commodity"] + string.Format("{0}/Mapping", entity.Id), content);
        }

        [TestMethod]
        public void should_create_a_mapping_on_the_commodity_entity()
        {
            var savedMapping =
                new DbSetRepository<MDM.CommodityMapping>(new NexusMappingContext()).FindOne(int.Parse(GetLocationHeader()[3]));
        ...
        public void should_return_the_location_of_the_entity()
        {
            ...
            Assert.IsTrue(parsedEntityId, "The Commodity id returned was not an integer");
