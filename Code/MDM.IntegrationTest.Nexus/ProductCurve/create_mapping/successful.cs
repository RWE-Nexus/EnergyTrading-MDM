namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_request_is_made_to_create_a_productcurve_mapping : IntegrationTestBase
    {
        ...
        {
            response = client.Post(ServiceUrl["ProductCurve"] + string.Format("{0}/Mapping", entity.Id), content);
        }

        [TestMethod]
        public void should_create_a_mapping_on_the_productcurve_entity()
        {
            var savedMapping =
                new DbSetRepository<MDM.ProductCurveMapping>(new NexusMappingContext()).FindOne(int.Parse(GetLocationHeader()[3]));
        ...
        public void should_return_the_location_of_the_entity()
        {
            ...
            Assert.IsTrue(parsedEntityId, "The ProductCurve id returned was not an integer");
