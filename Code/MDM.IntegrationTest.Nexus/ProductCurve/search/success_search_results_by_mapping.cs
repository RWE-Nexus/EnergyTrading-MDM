namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_search_for_a_productcurve_is_made_with_a_mapping_value_and_results_are_found : IntegrationTestBase
    {
        ...
        public void should_return_the_relevant_search_results()
        {
            ...
            List<RWEST.Nexus.MDM.Contracts.ProductCurve> result =
                feed.Items.Select(syndicationItem => (XmlSyndicationContent)syndicationItem.Content).Select(
                    syndic => syndic.ReadContent<RWEST.Nexus.MDM.Contracts.ProductCurve>()).ToList();
        ...
        {
            response = client.Post(ServiceUrl["ProductCurve"] + "search", content);
        }

        protected static void Establish_context()
        {
            entity1 = ProductCurveData.CreateBasicEntityWithOneMapping();
            entity2 = ProductCurveData.CreateBasicEntityWithOneMapping();
    ...
    [TestClass]
    public class when_a_search_for_a_productcurve_is_made_with_a_mapping_value_and_a_system_name_and_results_are_found : IntegrationTestBase
    {
        private static HttpClient client;
