namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductTypeMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<ProductTypeMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<ProductTypeMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            //ProductType.AddDetails(details);
            producttype.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.ProductTypeDetails();

            mappingEngine.Setup(x => x.Map<ProductTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<ProductType, RWEST.Nexus.MDM.Contracts.ProductTypeDetails>(producttype)).Returns(cDetails);
            validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);

            var list = new List<ProductTypeMapping> { mapping };
            repository.Setup(x => x.Queryable<ProductTypeMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<ProductTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<ProductType, RWEST.Nexus.MDM.Contracts.ProductTypeDetails>(producttype));
            repository.Verify(x => x.Queryable<ProductTypeMapping>());
