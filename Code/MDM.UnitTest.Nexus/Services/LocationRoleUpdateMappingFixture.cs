namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class LocationRoleUpdateMappingFixture
    {
        [TestMethod]
        ...
        public void VersionConflict()
        {
            ...
            var mapping = new LocationRoleMapping { LocationRole = new MDM.LocationRole() { Timestamp = BitConverter.GetBytes(25L)} };

            validatorFactory.Setup(x => x.IsValid(It.IsAny<AmendMappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            repository.Setup(x => x.FindOne<LocationRoleMapping>(12)).Returns(mapping);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var m1 = new LocationRoleMapping { Id = mappingId, System = s1, MappingValue = "1", Version = BitConverter.GetBytes(34L), Validity = new DateRange(start, DateUtility.MaxDate) };			
            var m2 = new LocationRoleMapping { Id = mappingId, System = s1, MappingValue = "1", Validity = new DateRange(start, finish) };

            // NB We deliberately bypasses the business logic
            var entity = new MDM.LocationRole();
            m1.LocationRole = entity;
            entity.Mappings.Add(m1);

            validatorFactory.Setup(x => x.IsValid(It.IsAny<AmendMappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            repository.Setup(x => x.FindOne<LocationRoleMapping>(mappingId)).Returns(m1);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, LocationRoleMapping>(identifier)).Returns(m2);
            ...
            // NB Don't verify result of Update - already covered by LocationRoleMappingFixture
