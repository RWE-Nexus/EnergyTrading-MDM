namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class PortfolioUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<Portfolio>(1)).Returns(new Portfolio());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.PortfolioDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.Portfolio { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<Portfolio>();
            details.Id = 1;
            var entity = ObjectMother.Create<Portfolio>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<Portfolio>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new PortfolioMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<Portfolio>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<Portfolio>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.PortfolioDetails, Portfolio>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, PortfolioMapping>(identifier)).Returns(mapping);
