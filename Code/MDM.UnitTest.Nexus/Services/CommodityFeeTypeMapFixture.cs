namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class CommodityFeeTypeMapFixture
    {
        [TestMethod]
        ...
        public void NullRequestErrors()
        {
            ...
            var service = new CommodityFeeTypeService(validatorFactory.Object, mappingEngine.Object, repository.Object, searchCache.Object);
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<CommodityFeeTypeMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<CommodityFeeTypeMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new CommodityFeeTypeMapping
            {
                System = system,
                MappingValue = "A"
            };
            var commodityfeetype = new CommodityFeeType
            {
                Id = 1,
                Validity = new DateRange(start, finish)
            };
            //CommodityFeeType.AddDetails(details);
            commodityfeetype.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.CommodityFeeTypeDetails();

            mappingEngine.Setup(x => x.Map<CommodityFeeTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<CommodityFeeType, RWEST.Nexus.MDM.Contracts.CommodityFeeTypeDetails>(commodityfeetype)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<CommodityFeeTypeMapping> { mapping };
            repository.Setup(x => x.Queryable<CommodityFeeTypeMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<CommodityFeeTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<CommodityFeeType, RWEST.Nexus.MDM.Contracts.CommodityFeeTypeDetails>(commodityfeetype));
            repository.Verify(x => x.Queryable<CommodityFeeTypeMapping>());
