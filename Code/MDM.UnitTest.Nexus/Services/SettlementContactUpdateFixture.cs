namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class SettlementContactUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<SettlementContact>(1)).Returns(new SettlementContact());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.SettlementContactDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.SettlementContact { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<SettlementContact>();
            details.Id = 1;
            var entity = ObjectMother.Create<SettlementContact>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<SettlementContact>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SettlementContactDetails, SettlementContact>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, PartyAccountabilityMapping>(identifier)).Returns(mapping);
