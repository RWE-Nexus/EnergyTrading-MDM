namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class PartyOverrideUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<PartyOverride>(1)).Returns(new PartyOverride());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.PartyOverrideDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.PartyOverride { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<PartyOverride>();
            details.Id = 1;
            var entity = ObjectMother.Create<PartyOverride>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<PartyOverride>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new PartyOverrideMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<PartyOverride>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<PartyOverride>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.PartyOverrideDetails, PartyOverride>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, PartyOverrideMapping>(identifier)).Returns(mapping);
