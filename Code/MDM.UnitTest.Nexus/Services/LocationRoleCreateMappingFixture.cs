namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class LocationRoleCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            var locationrole = new MDM.LocationRole();
            repository.Setup(x => x.FindOne<MDM.LocationRole>(12)).Returns(locationrole);

            // Act
            var candidate = (LocationRoleMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(locationrole));
