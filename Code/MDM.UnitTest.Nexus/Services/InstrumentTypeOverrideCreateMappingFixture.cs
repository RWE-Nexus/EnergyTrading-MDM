namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class InstrumentTypeOverrideCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.InstrumentTypeOverride>(12)).Returns(instrumenttypeoverride);

            // Act
            var candidate = (InstrumentTypeOverrideMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(instrumenttypeoverride));
