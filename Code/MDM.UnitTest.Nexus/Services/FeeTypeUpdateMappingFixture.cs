namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class FeeTypeUpdateMappingFixture
    {
        [TestMethod]
        ...
        public void VersionConflict()
        {
            ...
            var mapping = new FeeTypeMapping { FeeType = new MDM.FeeType() { Timestamp = BitConverter.GetBytes(25L) } };
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<AmendMappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            repository.Setup(x => x.FindOne<FeeTypeMapping>(12)).Returns(mapping);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var m1 = new FeeTypeMapping { Id = mappingId, System = s1, MappingValue = "1", Version = BitConverter.GetBytes(34L), Validity = new DateRange(start, DateUtility.MaxDate) };			
            var m2 = new FeeTypeMapping { Id = mappingId, System = s1, MappingValue = "1", Validity = new DateRange(start, finish) };
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<AmendMappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            repository.Setup(x => x.FindOne<FeeTypeMapping>(mappingId)).Returns(m1);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, FeeTypeMapping>(identifier)).Returns(m2);
            ...
            // NB Don't verify result of Update - already covered by FeeTypeMappingFixture
