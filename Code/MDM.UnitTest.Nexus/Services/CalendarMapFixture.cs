namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class CalendarMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<CalendarMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<CalendarMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            //Calendar.AddDetails(details);
            calendar.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.CalendarDetails();

            mappingEngine.Setup(x => x.Map<CalendarMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<Calendar, RWEST.Nexus.MDM.Contracts.CalendarDetails>(calendar)).Returns(cDetails);
            validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);

            var list = new List<CalendarMapping> { mapping };
            repository.Setup(x => x.Queryable<CalendarMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<CalendarMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<Calendar, RWEST.Nexus.MDM.Contracts.CalendarDetails>(calendar));
            repository.Verify(x => x.Queryable<CalendarMapping>());
