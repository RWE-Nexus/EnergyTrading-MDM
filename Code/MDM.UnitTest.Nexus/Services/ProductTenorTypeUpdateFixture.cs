namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductTenorTypeUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<ProductTenorType>(1)).Returns(new ProductTenorType());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.ProductTenorTypeDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.ProductTenorType { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<ProductTenorType>();
            details.Id = 1;
            var entity = ObjectMother.Create<ProductTenorType>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<ProductTenorType>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new ProductTenorTypeMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<ProductTenorType>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<ProductTenorType>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.ProductTenorTypeDetails, ProductTenorType>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, ProductTenorTypeMapping>(identifier)).Returns(mapping);
