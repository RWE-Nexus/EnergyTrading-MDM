namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class AgreementMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<AgreementMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<AgreementMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new AgreementMapping
            {
                System = system,
                MappingValue = "A"
            };
            ...
            //Agreement.AddDetails(details);
            agreement.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.AgreementDetails();

            mappingEngine.Setup(x => x.Map<AgreementMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<Agreement, RWEST.Nexus.MDM.Contracts.AgreementDetails>(agreement)).Returns(cDetails);
            validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<AgreementMapping> { mapping };
            repository.Setup(x => x.Queryable<AgreementMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<AgreementMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<Agreement, RWEST.Nexus.MDM.Contracts.AgreementDetails>(agreement));
            repository.Verify(x => x.Queryable<AgreementMapping>());
