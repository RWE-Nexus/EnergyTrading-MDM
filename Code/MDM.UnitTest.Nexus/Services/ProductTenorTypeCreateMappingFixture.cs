namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductTenorTypeCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            var producttenortype = new MDM.ProductTenorType();
            repository.Setup(x => x.FindOne<MDM.ProductTenorType>(12)).Returns(producttenortype);

            // Act
            var candidate = (ProductTenorTypeMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(producttenortype));
