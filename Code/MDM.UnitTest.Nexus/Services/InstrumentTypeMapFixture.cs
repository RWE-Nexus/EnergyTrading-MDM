namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class InstrumentTypeMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<InstrumentTypeMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<InstrumentTypeMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new InstrumentTypeMapping
            {
                System = system,
                MappingValue = "A"
            };
            ...
            //InstrumentType.AddDetails(details);
            instrumenttype.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.InstrumentTypeDetails();

            mappingEngine.Setup(x => x.Map<InstrumentTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<InstrumentType, RWEST.Nexus.MDM.Contracts.InstrumentTypeDetails>(instrumenttype)).Returns(cDetails);
            validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);

            var list = new List<InstrumentTypeMapping> { mapping };
            repository.Setup(x => x.Queryable<InstrumentTypeMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<InstrumentTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<InstrumentType, RWEST.Nexus.MDM.Contracts.InstrumentTypeDetails>(instrumenttype));
            repository.Verify(x => x.Queryable<InstrumentTypeMapping>());
