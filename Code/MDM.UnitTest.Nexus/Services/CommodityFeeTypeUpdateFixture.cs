namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class CommodityFeeTypeUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<CommodityFeeType>(1)).Returns(new CommodityFeeType());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<RWEST.Nexus.MDM.Contracts.CommodityFeeType>(), It.IsAny<IList<IRule>>())).Returns(true);

            var service = new CommodityFeeTypeService(validatorFactory.Object, mappingEngine.Object, repository.Object, searchCache.Object);

            var cd = new RWEST.Nexus.MDM.Contracts.CommodityFeeTypeDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.CommodityFeeType { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<CommodityFeeType>();
            details.Id = 1;
            var entity = ObjectMother.Create<CommodityFeeType>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<CommodityFeeType>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new CommodityFeeTypeMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<CommodityFeeType>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<CommodityFeeType>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.CommodityFeeTypeDetails, CommodityFeeType>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, CommodityFeeTypeMapping>(identifier)).Returns(mapping);
