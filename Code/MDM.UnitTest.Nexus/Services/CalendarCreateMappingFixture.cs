namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class CalendarCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.Calendar>(12)).Returns(calendar);

            // Act
            var candidate = (CalendarMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(calendar));
