namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class HierarchyMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<HierarchyMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<HierarchyMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            //Hierarchy.AddDetails(details);
            hierarchy.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.HierarchyDetails();

            mappingEngine.Setup(x => x.Map<HierarchyMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<Hierarchy, RWEST.Nexus.MDM.Contracts.HierarchyDetails>(hierarchy)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<HierarchyMapping> { mapping };
            repository.Setup(x => x.Queryable<HierarchyMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<HierarchyMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<Hierarchy, RWEST.Nexus.MDM.Contracts.HierarchyDetails>(hierarchy));
            repository.Verify(x => x.Queryable<HierarchyMapping>());
