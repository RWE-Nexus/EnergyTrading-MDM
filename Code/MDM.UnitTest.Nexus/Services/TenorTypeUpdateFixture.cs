namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class TenorTypeUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<TenorType>(1)).Returns(new TenorType());

            var service = new TenorTypeService(validatorFactory.Object, mappingEngine.Object, repository.Object, searchCache.Object);
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<RWEST.Nexus.MDM.Contracts.TenorType>(), It.IsAny<IList<IRule>>())).Returns(true);

            var service = new TenorTypeService(validatorFactory.Object, mappingEngine.Object, repository.Object, searchCache.Object);

            var cd = new RWEST.Nexus.MDM.Contracts.TenorTypeDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.TenorType { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<TenorType>();
            details.Id = 1;
            var entity = ObjectMother.Create<TenorType>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<TenorType>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new TenorTypeMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<TenorType>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<TenorType>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.TenorTypeDetails, TenorType>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, TenorTypeMapping>(identifier)).Returns(mapping);
