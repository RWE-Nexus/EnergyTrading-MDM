namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class BookDefaultMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            // Assert
            repository.Verify(x => x.Queryable<BookDefaultMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.BookDefaultDetails();

            mappingEngine.Setup(x => x.Map<BookDefaultMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<BookDefault, RWEST.Nexus.MDM.Contracts.BookDefaultDetails>(defaultportfolio)).Returns(cDetails);
            validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<BookDefaultMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<BookDefault, RWEST.Nexus.MDM.Contracts.BookDefaultDetails>(defaultportfolio));
            repository.Verify(x => x.Queryable<BookDefaultMapping>());
