namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class CommodityMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<CommodityMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<CommodityMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new CommodityMapping
            {
                System = system,
                MappingValue = "A"
            };
            ...
            //Commodity.AddDetails(details);
            commodity.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.CommodityDetails();

            mappingEngine.Setup(x => x.Map<CommodityMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<Commodity, RWEST.Nexus.MDM.Contracts.CommodityDetails>(commodity)).Returns(cDetails);
            validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);

            var list = new List<CommodityMapping> { mapping };
            repository.Setup(x => x.Queryable<CommodityMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<CommodityMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<Commodity, RWEST.Nexus.MDM.Contracts.CommodityDetails>(commodity));
            repository.Verify(x => x.Queryable<CommodityMapping>());
