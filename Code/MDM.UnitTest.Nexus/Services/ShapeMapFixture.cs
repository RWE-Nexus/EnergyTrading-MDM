namespace EnergyTrading.MDM.Test.Services
{
    ...
    public class ShapeMapFixture
    {
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<ShapeMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<ShapeMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            //Shape.AddDetails(details);
            shape.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.ShapeDetails();

            mappingEngine.Setup(x => x.Map<ShapeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<Shape, RWEST.Nexus.MDM.Contracts.ShapeDetails>(shape)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<ShapeMapping> { mapping };
            repository.Setup(x => x.Queryable<ShapeMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<ShapeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<Shape, RWEST.Nexus.MDM.Contracts.ShapeDetails>(shape));
            repository.Verify(x => x.Queryable<ShapeMapping>());
