namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ShapeUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<Shape>(1)).Returns(new Shape());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.ShapeDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.Shape { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<Shape>();
            details.Id = 1;
            var entity = ObjectMother.Create<Shape>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<Shape>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new ShapeMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<Shape>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<Shape>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.ShapeDetails, Shape>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, ShapeMapping>(identifier)).Returns(mapping);
