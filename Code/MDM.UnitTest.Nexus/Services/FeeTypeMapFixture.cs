namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class FeeTypeMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<FeeTypeMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<FeeTypeMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            //FeeType.AddDetails(details);
            feetype.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.FeeTypeDetails();

            mappingEngine.Setup(x => x.Map<FeeTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<FeeType, RWEST.Nexus.MDM.Contracts.FeeTypeDetails>(feetype)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<FeeTypeMapping> { mapping };
            repository.Setup(x => x.Queryable<FeeTypeMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<FeeTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<FeeType, RWEST.Nexus.MDM.Contracts.FeeTypeDetails>(feetype));
            repository.Verify(x => x.Queryable<FeeTypeMapping>());
