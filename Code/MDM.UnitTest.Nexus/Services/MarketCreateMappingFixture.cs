namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class MarketCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            var market = new MDM.Market();
            repository.Setup(x => x.FindOne<MDM.Market>(12)).Returns(market);

            // Act
            var candidate = (MarketMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(market));
