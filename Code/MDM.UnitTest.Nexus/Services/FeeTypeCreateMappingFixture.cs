namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class FeeTypeCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.FeeType>(12)).Returns(feetype);

            // Act
            var candidate = (FeeTypeMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(feetype));
