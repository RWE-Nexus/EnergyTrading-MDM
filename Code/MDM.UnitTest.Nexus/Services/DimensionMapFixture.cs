namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class DimensionMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<DimensionMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<DimensionMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            //Dimension.AddDetails(details);
            dimension.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.DimensionDetails();

            mappingEngine.Setup(x => x.Map<DimensionMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<Dimension, RWEST.Nexus.MDM.Contracts.DimensionDetails>(dimension)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<DimensionMapping> { mapping };
            repository.Setup(x => x.Queryable<DimensionMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<DimensionMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<Dimension, RWEST.Nexus.MDM.Contracts.DimensionDetails>(dimension));
            repository.Verify(x => x.Queryable<DimensionMapping>());
