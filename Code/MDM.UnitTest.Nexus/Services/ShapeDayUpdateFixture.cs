namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ShapeDayUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<ShapeDay>(1)).Returns(new ShapeDay());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.ShapeDayDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.ShapeDay { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<ShapeDay>();
            details.Id = 1;
            var entity = ObjectMother.Create<ShapeDay>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<ShapeDay>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new ShapeDayMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<ShapeDay>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<ShapeDay>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.ShapeDayDetails, ShapeDay>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, ShapeDayMapping>(identifier)).Returns(mapping);
