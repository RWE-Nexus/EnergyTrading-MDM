namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class InstrumentTypeOverrideMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<InstrumentTypeOverrideMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<InstrumentTypeOverrideMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new InstrumentTypeOverrideMapping
            {
                System = system,
                MappingValue = "A"
            };
            ...
            //InstrumentTypeOverride.AddDetails(details);
            instrumenttypeoverride.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.InstrumentTypeOverrideDetails();

            mappingEngine.Setup(x => x.Map<InstrumentTypeOverrideMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<InstrumentTypeOverride, RWEST.Nexus.MDM.Contracts.InstrumentTypeOverrideDetails>(instrumenttypeoverride)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<InstrumentTypeOverrideMapping> { mapping };
            repository.Setup(x => x.Queryable<InstrumentTypeOverrideMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<InstrumentTypeOverrideMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<InstrumentTypeOverride, RWEST.Nexus.MDM.Contracts.InstrumentTypeOverrideDetails>(instrumenttypeoverride));
            repository.Verify(x => x.Queryable<InstrumentTypeOverrideMapping>());
