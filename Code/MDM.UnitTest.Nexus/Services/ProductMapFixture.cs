namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<ProductMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<ProductMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            product.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.ProductDetails {LotSize = 100};

            mappingEngine.Setup(x => x.Map<ProductMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<Product, RWEST.Nexus.MDM.Contracts.ProductDetails>(product)).Returns(cDetails);
            validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);

            var list = new List<ProductMapping> { mapping };
            repository.Setup(x => x.Queryable<ProductMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<ProductMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<Product, RWEST.Nexus.MDM.Contracts.ProductDetails>(product));
            repository.Verify(x => x.Queryable<ProductMapping>());
            ...
            Assert.AreEqual(cDetails.LotSize, 100);
