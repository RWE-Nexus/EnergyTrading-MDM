namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class CalendarCreateFixture
    {
        [TestMethod]
        ...
        public void ValidContractIsSaved()
        {
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.Calendar, Calendar>(contract)).Returns(calendar);

            // Act
            var expected = service.Create(contract);

            // Assert
            Assert.AreSame(expected, calendar, "Calendar differs");
            repository.Verify(x => x.Add(calendar));
