namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class TenorTypeMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<TenorTypeMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<TenorTypeMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new TenorTypeMapping
            {
                System = system,
                MappingValue = "A"
            };
            ...
            //TenorType.AddDetails(details);
            tenortype.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.TenorTypeDetails();

            mappingEngine.Setup(x => x.Map<TenorTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<TenorType, RWEST.Nexus.MDM.Contracts.TenorTypeDetails>(tenortype)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<TenorTypeMapping> { mapping };
            repository.Setup(x => x.Queryable<TenorTypeMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<TenorTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<TenorType, RWEST.Nexus.MDM.Contracts.TenorTypeDetails>(tenortype));
            repository.Verify(x => x.Queryable<TenorTypeMapping>());
