namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductScotaUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<ProductScota>(1)).Returns(new ProductScota());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<RWEST.Nexus.MDM.Contracts.ProductScota>(), It.IsAny<IList<IRule>>())).Returns(true);

            var service = new ProductScotaService(validatorFactory.Object, mappingEngine.Object, repository.Object, searchCache.Object);

            var cd = new RWEST.Nexus.MDM.Contracts.ProductScotaDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.ProductScota { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<ProductScota>();
            details.Id = 1;
            var entity = ObjectMother.Create<ProductScota>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<ProductScota>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new ProductScotaMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<ProductScota>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<ProductScota>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.ProductScotaDetails, ProductScota>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, ProductScotaMapping>(identifier)).Returns(mapping);
