namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class PartyOverrideMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<PartyOverrideMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<PartyOverrideMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            //PartyOverride.AddDetails(details);
            partyoverride.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.PartyOverrideDetails();

            mappingEngine.Setup(x => x.Map<PartyOverrideMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<PartyOverride, RWEST.Nexus.MDM.Contracts.PartyOverrideDetails>(partyoverride)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<PartyOverrideMapping> { mapping };
            repository.Setup(x => x.Queryable<PartyOverrideMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<PartyOverrideMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<PartyOverride, RWEST.Nexus.MDM.Contracts.PartyOverrideDetails>(partyoverride));
            repository.Verify(x => x.Queryable<PartyOverrideMapping>());
