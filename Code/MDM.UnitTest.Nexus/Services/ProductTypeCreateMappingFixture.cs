namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductTypeCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.ProductType>(12)).Returns(producttype);

            // Act
            var candidate = (ProductTypeMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(producttype));
