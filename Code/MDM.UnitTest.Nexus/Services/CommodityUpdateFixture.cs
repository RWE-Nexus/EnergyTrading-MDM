namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class CommodityUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<Commodity>(1)).Returns(new Commodity());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.CommodityDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.Commodity { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<Commodity>();
            details.Id = 1;
            var entity = ObjectMother.Create<Commodity>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<Commodity>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new CommodityMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<Commodity>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<Commodity>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.CommodityDetails, Commodity>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, CommodityMapping>(identifier)).Returns(mapping);
