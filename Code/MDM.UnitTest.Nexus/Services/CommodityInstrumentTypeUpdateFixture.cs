namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class CommodityInstrumentTypeUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<CommodityInstrumentType>(1)).Returns(new CommodityInstrumentType());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<RWEST.Nexus.MDM.Contracts.CommodityInstrumentType>(), It.IsAny<IList<IRule>>())).Returns(true);

            var service = new CommodityInstrumentTypeService(validatorFactory.Object, mappingEngine.Object, repository.Object, searchCache.Object);

            var cd = new RWEST.Nexus.MDM.Contracts.CommodityInstrumentTypeDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.CommodityInstrumentType { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<CommodityInstrumentType>();
            details.Id = 1;
            var entity = ObjectMother.Create<CommodityInstrumentType>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<CommodityInstrumentType>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new CommodityInstrumentTypeMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<CommodityInstrumentType>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<CommodityInstrumentType>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.CommodityInstrumentTypeDetails, CommodityInstrumentType>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, CommodityInstrumentTypeMapping>(identifier)).Returns(mapping);
