namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class BrokerRateCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.BrokerRate>(12)).Returns(brokerrate);

            // Act
            var candidate = (BrokerRateMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(brokerrate));
