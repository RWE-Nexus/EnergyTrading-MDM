namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class CommodityInstrumentTypeMapFixture
    {
        [TestMethod]
        ...
        public void NullRequestErrors()
        {
            ...
            var service = new CommodityInstrumentTypeService(validatorFactory.Object, mappingEngine.Object, repository.Object, searchCache.Object);
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<CommodityInstrumentTypeMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<CommodityInstrumentTypeMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new CommodityInstrumentTypeMapping
            {
                System = system,
                MappingValue = "A"
            };
            ...
            //CommodityInstrumentType.AddDetails(details);
            commodityinstrumenttype.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.CommodityInstrumentTypeDetails();

            mappingEngine.Setup(x => x.Map<CommodityInstrumentTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<CommodityInstrumentType, RWEST.Nexus.MDM.Contracts.CommodityInstrumentTypeDetails>(commodityinstrumenttype)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<CommodityInstrumentTypeMapping> { mapping };
            repository.Setup(x => x.Queryable<CommodityInstrumentTypeMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<CommodityInstrumentTypeMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<CommodityInstrumentType, RWEST.Nexus.MDM.Contracts.CommodityInstrumentTypeDetails>(commodityinstrumenttype));
            repository.Verify(x => x.Queryable<CommodityInstrumentTypeMapping>());
