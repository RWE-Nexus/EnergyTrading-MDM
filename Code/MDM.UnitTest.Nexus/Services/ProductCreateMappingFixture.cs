namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.Product>(12)).Returns(product);

            // Act
            var candidate = (ProductMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(product));
