namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductTypeInstanceMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<ProductTypeInstanceMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<ProductTypeInstanceMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new ProductTypeInstanceMapping
            {
                System = system,
                MappingValue = "A"
            };
            ...
            //ProductTypeInstance.AddDetails(details);
            producttypeinstance.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.ProductTypeInstanceDetails();

            mappingEngine.Setup(x => x.Map<ProductTypeInstanceMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<ProductTypeInstance, RWEST.Nexus.MDM.Contracts.ProductTypeInstanceDetails>(producttypeinstance)).Returns(cDetails);
            validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);

            var list = new List<ProductTypeInstanceMapping> { mapping };
            repository.Setup(x => x.Queryable<ProductTypeInstanceMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<ProductTypeInstanceMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<ProductTypeInstance, RWEST.Nexus.MDM.Contracts.ProductTypeInstanceDetails>(producttypeinstance));
            repository.Verify(x => x.Queryable<ProductTypeInstanceMapping>());
