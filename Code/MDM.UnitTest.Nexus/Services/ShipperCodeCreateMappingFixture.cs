namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ShipperCodeCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.ShipperCode>(12)).Returns(shippercode);

            // Act
            var candidate = (ShipperCodeMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(shippercode));
