namespace EnergyTrading.MDM.Test.Services
{
    ...
    public class LocationUpdateFixture : Fixture
    {
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<RWEST.Nexus.MDM.Contracts.Location>(), It.IsAny<IList<IRule>>())).Returns(true);

            var service = new LocationService(validatorFactory.Object, mappingEngine.Object, repository.Object, searchCache.Object);

            var cd = new RWEST.Nexus.MDM.Contracts.LocationDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.Location { Details = cd, Nexus = nexus };
        ...
        public void ValidDetailsSaved()
        {
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.LocationDetails, Location>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, LocationMapping>(identifier)).Returns(mapping);
