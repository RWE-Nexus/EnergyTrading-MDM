namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class FeeTypeUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<FeeType>(1)).Returns(new FeeType());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.FeeTypeDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.FeeType { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<FeeType>();
            details.Id = 1;
            var entity = ObjectMother.Create<FeeType>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<FeeType>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new FeeTypeMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<FeeType>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<FeeType>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.FeeTypeDetails, FeeType>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, FeeTypeMapping>(identifier)).Returns(mapping);
