namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ProductCurveMapFixture
    {
        [TestMethod]
        ...
        public void UnsuccessfulMatchReturnsNotFound()
        {
            ...
            var list = new List<ProductCurveMapping>();
            ...
            // Assert
            repository.Verify(x => x.Queryable<ProductCurveMapping>());
        ...
        public void SuccessMatch()
        {
            ...
            var mapping = new ProductCurveMapping
            {
                System = system,
                MappingValue = "A"
            };
            ...
            //ProductCurve.AddDetails(details);
            productcurve.ProcessMapping(mapping);
            ...
            var cDetails = new RWEST.Nexus.MDM.Contracts.ProductCurveDetails();

            mappingEngine.Setup(x => x.Map<ProductCurveMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping)).Returns(identifier);
            mappingEngine.Setup(x => x.Map<ProductCurve, RWEST.Nexus.MDM.Contracts.ProductCurveDetails>(productcurve)).Returns(cDetails);
			validatorFactory.Setup(x => x.IsValid(It.IsAny<MappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);


            var list = new List<ProductCurveMapping> { mapping };
            repository.Setup(x => x.Queryable<ProductCurveMapping>()).Returns(list.AsQueryable());
            ...
            // Assert
            mappingEngine.Verify(x => x.Map<ProductCurveMapping, RWEST.Nexus.MDM.Contracts.NexusId>(mapping));
            mappingEngine.Verify(x => x.Map<ProductCurve, RWEST.Nexus.MDM.Contracts.ProductCurveDetails>(productcurve));
            repository.Verify(x => x.Queryable<ProductCurveMapping>());
