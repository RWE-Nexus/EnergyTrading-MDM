namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class PortfolioCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.Portfolio>(12)).Returns(portfolio);

            // Act
            var candidate = (PortfolioMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(portfolio));
