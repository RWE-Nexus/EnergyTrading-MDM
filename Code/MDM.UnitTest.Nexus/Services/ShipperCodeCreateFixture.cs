namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class ShipperCodeCreateFixture
    {
        [TestMethod]
        ...
        public void ValidContractIsSaved()
        {
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.ShipperCode, ShipperCode>(contract)).Returns(shippercode);

            // Act
            var expected = service.Create(contract);

            // Assert
            Assert.AreSame(expected, shippercode, "ShipperCode differs");
            repository.Verify(x => x.Add(shippercode));
