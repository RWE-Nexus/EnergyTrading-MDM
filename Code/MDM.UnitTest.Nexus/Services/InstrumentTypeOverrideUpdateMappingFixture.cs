namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class InstrumentTypeOverrideUpdateMappingFixture
    {
        [TestMethod]
        ...
        public void VersionConflict()
        {
            ...
            var mapping = new InstrumentTypeOverrideMapping { InstrumentTypeOverride = new MDM.InstrumentTypeOverride() { Timestamp = BitConverter.GetBytes(25L) } };
            ...
            validatorFactory.Setup(x => x.IsValid(It.IsAny<AmendMappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            repository.Setup(x => x.FindOne<InstrumentTypeOverrideMapping>(12)).Returns(mapping);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var m1 = new InstrumentTypeOverrideMapping { Id = mappingId, System = s1, MappingValue = "1", Version = BitConverter.GetBytes(34L), Validity = new DateRange(start, DateUtility.MaxDate) };			
            var m2 = new InstrumentTypeOverrideMapping { Id = mappingId, System = s1, MappingValue = "1", Validity = new DateRange(start, finish) };

            // NB We deliberately bypasses the business logic
            var entity = new MDM.InstrumentTypeOverride();
            m1.InstrumentTypeOverride = entity;
            entity.Mappings.Add(m1);

            validatorFactory.Setup(x => x.IsValid(It.IsAny<AmendMappingRequest>(), It.IsAny<IList<IRule>>())).Returns(true);
            repository.Setup(x => x.FindOne<InstrumentTypeOverrideMapping>(mappingId)).Returns(m1);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, InstrumentTypeOverrideMapping>(identifier)).Returns(m2);
            ...
            // NB Don't verify result of Update - already covered by InstrumentTypeOverrideMappingFixture
