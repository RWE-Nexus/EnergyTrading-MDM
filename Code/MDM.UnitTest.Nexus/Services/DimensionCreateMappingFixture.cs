namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class DimensionCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.Dimension>(12)).Returns(dimension);

            // Act
            var candidate = (DimensionMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(dimension));
