namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class BookDefaultUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<BookDefault>(1)).Returns(new BookDefault());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.BookDefaultDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.BookDefault { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<BookDefault>();
            details.Id = 1;
            var entity = ObjectMother.Create<BookDefault>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<BookDefault>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.BookDefaultDetails, BookDefault>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, BookDefaultMapping>(identifier)).Returns(mapping);
