namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class BookDefaultCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.BookDefault>(12)).Returns(defaultportfolio);

            // Act
            var candidate = (BookDefaultMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(defaultportfolio));
