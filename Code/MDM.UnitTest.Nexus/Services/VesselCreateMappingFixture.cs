namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class VesselCreateMappingFixture : Fixture
    {
        [TestMethod]
        ...
        public void ValidContractAdded()
        {
            ...
            repository.Setup(x => x.FindOne<MDM.Vessel>(12)).Returns(vessel);

            // Act
            var candidate = (VesselMapping)service.CreateMapping(message);

            // Assert
            Assert.AreSame(mapping, candidate);
            repository.Verify(x => x.Save(vessel));
