namespace EnergyTrading.MDM.Test.Services
{
    ...
    [TestClass]
    public class InstrumentTypeOverrideUpdateFixture : Fixture
    {
        [TestMethod]
        ...
        public void NullContractInvalid()
        {
            ...
            repository.Setup(x => x.FindOne<InstrumentTypeOverride>(1)).Returns(new InstrumentTypeOverride());
        ...
        public void EarlierVersionRaisesVersionConflict()
        {
            ...
            var cd = new RWEST.Nexus.MDM.Contracts.InstrumentTypeOverrideDetails();
            var nexus = new RWEST.Nexus.MDM.Contracts.SystemData { StartDate = new DateTime(2012, 1, 1) };
            var contract = new RWEST.Nexus.MDM.Contracts.InstrumentTypeOverride { Details = cd, Nexus = nexus };

            var details = ObjectMother.Create<InstrumentTypeOverride>();
            details.Id = 1;
            var entity = ObjectMother.Create<InstrumentTypeOverride>();
            entity.Id = 2;
            entity.AddDetails(details);

            repository.Setup(x => x.FindOne<InstrumentTypeOverride>(1)).Returns(entity);
        ...
        public void ValidDetailsSaved()
        {
            ...
            var mapping = new InstrumentTypeOverrideMapping { System = system, MappingValue = "A" };
            var d1 = ObjectMother.Create<InstrumentTypeOverride>();
            d1.Id = 1;
            ...
            var d2 = ObjectMother.Create<InstrumentTypeOverride>(); 
            ...
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.InstrumentTypeOverrideDetails, InstrumentTypeOverride>(cd)).Returns(d2);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.SystemData, DateRange>(nexus)).Returns(range);
            mappingEngine.Setup(x => x.Map<RWEST.Nexus.MDM.Contracts.NexusId, InstrumentTypeOverrideMapping>(identifier)).Returns(mapping);
