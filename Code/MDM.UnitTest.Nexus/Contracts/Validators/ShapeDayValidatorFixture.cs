namespace EnergyTrading.MDM.Test.Contracts.Validators
{
    ...
    using ShapeDay = RWEST.Nexus.MDM.Contracts.ShapeDay;

    [TestClass]
    public partial class ShapeDayValidatorFixture : Fixture
    {
        [TestMethod]
        public void ValidatorResolution()
        {
            ...
            var config = new ShapeDayConfiguration(container);
            config.Configure();

            var validator = container.Resolve<IValidator<ShapeDay>>("shapeday");
        ...
        [TestMethod]
        public void ValidShapeDayPasses()
        {
            // Assert
            ...
            var validator = new ShapeDayValidator(validatorEngine.Object, repository);

            var shapeday = new ShapeDay { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { identifier } };
            this.AddRelatedEntities(shapeday);

            // Act
            var violations = new List<IRule>();
            var result = validator.IsValid(shapeday, violations);
        ...
        public void OverlapsRangeFails()
        {
            ...
            var system = new MDM.SourceSystem { Name = "Test" };
            var shapedayMapping = new ShapeDayMapping { System = system, MappingValue = "1", Validity = validity };

            var list = new List<ShapeDayMapping> { shapedayMapping };
            var repository = new Mock<IRepository>();
            repository.Setup(x => x.Queryable<ShapeDayMapping>()).Returns(list.AsQueryable());
            ...
            var identifierValidator = new NexusIdValidator<ShapeDayMapping>(repository.Object);
            var validatorEngine = new Mock<IValidatorEngine>();
            ...
            var validator = new ShapeDayValidator(validatorEngine.Object, repository.Object);

            var shapeday = new ShapeDay { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { overlapsRangeIdentifier } };
        ...
        public void BadSystemFails()
        {
            ...
            var shapeday = new ShapeDay { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { badSystemIdentifier } };
		...
		partial void AddRelatedEntities(RWEST.Nexus.MDM.Contracts.ShapeDay contract);
