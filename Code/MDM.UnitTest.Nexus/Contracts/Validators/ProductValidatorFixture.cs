namespace EnergyTrading.MDM.Test.Contracts.Validators
{
    ...
    using Product = RWEST.Nexus.MDM.Contracts.Product;

    [TestClass]
    public partial class ProductValidatorFixture : Fixture
    {
        [TestMethod]
        public void ValidatorResolution()
        {
            ...
            var config = new ProductConfiguration(container);
            config.Configure();

            var validator = container.Resolve<IValidator<Product>>("product");
        ...
        [TestMethod]
        public void ValidProductPasses()
        {
            // Assert
            ...
            var validator = new ProductValidator(validatorEngine.Object, repository);

            var product = new Product { Details = new RWEST.Nexus.MDM.Contracts.ProductDetails{Name = "Test"}, Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { identifier } };
            this.AddRelatedEntities(product);

            // Act
            var violations = new List<IRule>();
            var result = validator.IsValid(product, violations);
        ...
        public void OverlapsRangeFails()
        {
            ...
            var system = new MDM.SourceSystem { Name = "Test" };
            var productMapping = new ProductMapping { System = system, MappingValue = "1", Validity = validity };

            var list = new List<ProductMapping> { productMapping };
            var repository = new Mock<IRepository>();
            repository.Setup(x => x.Queryable<ProductMapping>()).Returns(list.AsQueryable());
            ...
            var identifierValidator = new NexusIdValidator<ProductMapping>(repository.Object);
            var validatorEngine = new Mock<IValidatorEngine>();
            ...
            var validator = new ProductValidator(validatorEngine.Object, repository.Object);

            var product = new Product { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { overlapsRangeIdentifier } };
        ...
        public void BadSystemFails()
        {
            ...
            var product = new Product { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { badSystemIdentifier } };
