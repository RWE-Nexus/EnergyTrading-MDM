namespace EnergyTrading.MDM.Test.Contracts.Validators
{
    ...
    using InstrumentType = RWEST.Nexus.MDM.Contracts.InstrumentType;

    [TestClass]
    public partial class InstrumentTypeValidatorFixture : Fixture
    {
        [TestMethod]
        public void ValidatorResolution()
        {
            ...
            var config = new InstrumentTypeConfiguration(container);
            config.Configure();

            var validator = container.Resolve<IValidator<InstrumentType>>("instrumenttype");
        ...
        [TestMethod]
        public void ValidInstrumentTypePasses()
        {
            // Assert
            ...
            var validator = new InstrumentTypeValidator(validatorEngine.Object, repository);

            var instrumenttype = new InstrumentType { Details = new RWEST.Nexus.MDM.Contracts.InstrumentTypeDetails{Name = "Test"}, Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { identifier } };
            this.AddRelatedEntities(instrumenttype);

            // Act
            var violations = new List<IRule>();
            var result = validator.IsValid(instrumenttype, violations);
        ...
        public void OverlapsRangeFails()
        {
            ...
            var system = new MDM.SourceSystem { Name = "Test" };
            var instrumenttypeMapping = new InstrumentTypeMapping { System = system, MappingValue = "1", Validity = validity };

            var list = new List<InstrumentTypeMapping> { instrumenttypeMapping };
            var repository = new Mock<IRepository>();
            repository.Setup(x => x.Queryable<InstrumentTypeMapping>()).Returns(list.AsQueryable());
            ...
            var identifierValidator = new NexusIdValidator<InstrumentTypeMapping>(repository.Object);
            var validatorEngine = new Mock<IValidatorEngine>();
            ...
            var validator = new InstrumentTypeValidator(validatorEngine.Object, repository.Object);

            var instrumenttype = new InstrumentType { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { overlapsRangeIdentifier } };
        ...
        public void BadSystemFails()
        {
            ...
            var instrumenttype = new InstrumentType { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { badSystemIdentifier } };
		...
		partial void AddRelatedEntities(RWEST.Nexus.MDM.Contracts.InstrumentType contract);
