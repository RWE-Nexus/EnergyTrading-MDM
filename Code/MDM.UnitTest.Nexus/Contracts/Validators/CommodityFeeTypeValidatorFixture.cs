namespace EnergyTrading.MDM.Test.Contracts.Validators
{
    ...
    using CommodityFeeType = RWEST.Nexus.MDM.Contracts.CommodityFeeType;

    [TestClass]
    public partial class CommodityFeeTypeValidatorFixture : Fixture
    {
        [TestMethod]
        public void ValidatorResolution()
        {
            ...
            var config = new CommodityFeeTypeConfiguration(container);
            config.Configure();

            var validator = container.Resolve<IValidator<CommodityFeeType>>("commodityfeetype");
        ...
        [TestMethod]
        public void ValidCommodityFeeTypePasses()
        {
            // Assert
            ...
            var validator = new CommodityFeeTypeValidator(validatorEngine.Object, repository);

            var commodityfeetype = new CommodityFeeType { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { identifier } };
            this.AddRelatedEntities(commodityfeetype);

            // Act
            var violations = new List<IRule>();
            var result = validator.IsValid(commodityfeetype, violations);
        ...
        public void OverlapsRangeFails()
        {
            ...
            var system = new MDM.SourceSystem { Name = "Test" };
            var commodityfeetypeMapping = new CommodityFeeTypeMapping { System = system, MappingValue = "1", Validity = validity };

            var list = new List<CommodityFeeTypeMapping> { commodityfeetypeMapping };
            var repository = new Mock<IRepository>();
            repository.Setup(x => x.Queryable<CommodityFeeTypeMapping>()).Returns(list.AsQueryable());
            ...
            var identifierValidator = new NexusIdValidator<CommodityFeeTypeMapping>(repository.Object);
            var validatorEngine = new Mock<IValidatorEngine>();
            ...
            var validator = new CommodityFeeTypeValidator(validatorEngine.Object, repository.Object);

            var commodityfeetype = new CommodityFeeType { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { overlapsRangeIdentifier } };
        ...
        public void BadSystemFails()
        {
            ...
            var commodityfeetype = new CommodityFeeType { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { badSystemIdentifier } };
		...
		partial void AddRelatedEntities(RWEST.Nexus.MDM.Contracts.CommodityFeeType contract);
