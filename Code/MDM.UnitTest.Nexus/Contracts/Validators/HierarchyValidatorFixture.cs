namespace EnergyTrading.MDM.Test.Contracts.Validators
{
    ...
    using Hierarchy = RWEST.Nexus.MDM.Contracts.Hierarchy;

    [TestClass]
    public partial class HierarchyValidatorFixture : Fixture
    {
        [TestMethod]
        public void ValidatorResolution()
        {
            ...
            var validator = container.Resolve<IValidator<Hierarchy>>("hierarchy");
        ...
        [TestMethod]
        public void ValidHierarchyPasses()
        {
            // Assert
            ...
            var validator = new HierarchyValidator(validatorEngine.Object, repository);

            var hierarchy = new Hierarchy { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { identifier } };
            this.AddRelatedEntities(hierarchy);

            // Act
            var violations = new List<IRule>();
            var result = validator.IsValid(hierarchy, violations);
        ...
        public void OverlapsRangeFails()
        {
            ...
            var system = new MDM.SourceSystem { Name = "Test" };
            var hierarchyMapping = new HierarchyMapping { System = system, MappingValue = "1", Validity = validity };

            var list = new List<HierarchyMapping> { hierarchyMapping };
            var repository = new Mock<IRepository>();
            repository.Setup(x => x.Queryable<HierarchyMapping>()).Returns(list.AsQueryable());
            ...
            var identifierValidator = new NexusIdValidator<HierarchyMapping>(repository.Object);
            var validatorEngine = new Mock<IValidatorEngine>();
            ...
            var validator = new HierarchyValidator(validatorEngine.Object, repository.Object);

            var hierarchy = new Hierarchy { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { overlapsRangeIdentifier } };
        ...
        public void BadSystemFails()
        {
            ...
            var hierarchy = new Hierarchy { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { badSystemIdentifier } };
		...
		partial void AddRelatedEntities(RWEST.Nexus.MDM.Contracts.Hierarchy contract);
