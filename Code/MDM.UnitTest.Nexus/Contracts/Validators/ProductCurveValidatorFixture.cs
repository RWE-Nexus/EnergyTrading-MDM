namespace EnergyTrading.MDM.Test.Contracts.Validators
{
    ...
    using ProductCurve = RWEST.Nexus.MDM.Contracts.ProductCurve;

    [TestClass]
    public partial class ProductCurveValidatorFixture : Fixture
    {
        [TestMethod]
        public void ValidatorResolution()
        {
            ...
            var config = new ProductCurveConfiguration(container);
            config.Configure();

            var validator = container.Resolve<IValidator<ProductCurve>>("productcurve");
        ...
        [TestMethod]
        public void ValidProductCurvePasses()
        {
            // Assert
            ...
            var validator = new ProductCurveValidator(validatorEngine.Object, repository);

            var productcurve = new ProductCurve { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { identifier } };
            this.AddRelatedEntities(productcurve);

            // Act
            var violations = new List<IRule>();
            var result = validator.IsValid(productcurve, violations);
        ...
        public void OverlapsRangeFails()
        {
            ...
            var system = new MDM.SourceSystem { Name = "Test" };
            var productcurveMapping = new ProductCurveMapping { System = system, MappingValue = "1", Validity = validity };

            var list = new List<ProductCurveMapping> { productcurveMapping };
            var repository = new Mock<IRepository>();
            repository.Setup(x => x.Queryable<ProductCurveMapping>()).Returns(list.AsQueryable());
            ...
            var identifierValidator = new NexusIdValidator<ProductCurveMapping>(repository.Object);
            var validatorEngine = new Mock<IValidatorEngine>();
            ...
            var validator = new ProductCurveValidator(validatorEngine.Object, repository.Object);

            var productcurve = new ProductCurve { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { overlapsRangeIdentifier } };
        ...
        public void BadSystemFails()
        {
            ...
            var productcurve = new ProductCurve { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { badSystemIdentifier } };
		...
		partial void AddRelatedEntities(RWEST.Nexus.MDM.Contracts.ProductCurve contract);
