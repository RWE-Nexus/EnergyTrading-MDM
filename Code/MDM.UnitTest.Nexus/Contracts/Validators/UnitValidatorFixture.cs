namespace EnergyTrading.MDM.Test.Contracts.Validators
{
    ...
    using Unit = RWEST.Nexus.MDM.Contracts.Unit;

    [TestClass]
    public partial class UnitValidatorFixture : Fixture
    {
        [TestMethod]
        public void ValidatorResolution()
        {
            ...
            var config = new UnitConfiguration(container);
            config.Configure();

            var validator = container.Resolve<IValidator<Unit>>("unit");
        ...
        public void ValidUnitPasses()
        {
            ...
            var validator = new UnitValidator(validatorEngine.Object, repository);

            var unit = new Unit { Details = new RWEST.Nexus.MDM.Contracts.UnitDetails{Name = "Test"}, Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { identifier } };
            this.AddRelatedEntities(unit);

            // Act
            var violations = new List<IRule>();
            var result = validator.IsValid(unit, violations);
        ...
        public void OverlapsRangeFails()
        {
            ...
            var system = new MDM.SourceSystem { Name = "Test" };
            var unitMapping = new UnitMapping { System = system, MappingValue = "1", Validity = validity };

            var list = new List<UnitMapping> { unitMapping };
            var repository = new Mock<IRepository>();
            ...
            var identifierValidator = new NexusIdValidator<UnitMapping>(repository.Object);
            var validatorEngine = new Mock<IValidatorEngine>();
            ...
            var validator = new UnitValidator(validatorEngine.Object, repository.Object);

            var unit = new Unit { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { overlapsRangeIdentifier } };
        ...
        public void BadSystemFails()
        {
            ...
            var unit = new Unit { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { badSystemIdentifier } };
		...
		partial void AddRelatedEntities(RWEST.Nexus.MDM.Contracts.Unit contract);
