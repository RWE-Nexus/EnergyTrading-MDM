namespace EnergyTrading.MDM.Test.Contracts.Validators
{
    ...
    using Shape = RWEST.Nexus.MDM.Contracts.Shape;

    [TestClass]
    public partial class ShapeValidatorFixture : Fixture
    {
        [TestMethod]
        public void ValidatorResolution()
        {
            ...
            var config = new ShapeConfiguration(container);
            config.Configure();

            var validator = container.Resolve<IValidator<Shape>>("shape");
        ...
        public void ValidShapePasses()
        {
            ...
            var validator = new ShapeValidator(validatorEngine.Object, repository);

            var shape = new Shape { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { identifier } };
            this.AddRelatedEntities(shape);

            // Act
            var violations = new List<IRule>();
            var result = validator.IsValid(shape, violations);
        ...
        public void OverlapsRangeFails()
        {
            ...
            var system = new MDM.SourceSystem { Name = "Test" };
            var shapeMapping = new ShapeMapping { System = system, MappingValue = "1", Validity = validity };

            var list = new List<ShapeMapping> { shapeMapping };
            var repository = new Mock<IRepository>();
            repository.Setup(x => x.Queryable<ShapeMapping>()).Returns(list.AsQueryable());
            ...
            var identifierValidator = new NexusIdValidator<ShapeMapping>(repository.Object);
            var validatorEngine = new Mock<IValidatorEngine>();
            ...
            var validator = new ShapeValidator(validatorEngine.Object, repository.Object);

            var shape = new Shape { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { overlapsRangeIdentifier } };
        ...
        public void BadSystemFails()
        {
            ...
            var shape = new Shape { Identifiers = new RWEST.Nexus.MDM.Contracts.NexusIdList { badSystemIdentifier } };
		...
		partial void AddRelatedEntities(RWEST.Nexus.MDM.Contracts.Shape contract);
